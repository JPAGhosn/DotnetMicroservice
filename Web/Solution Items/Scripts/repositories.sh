...
docker build \
  ...
  -f ../K8S/Repositories/External/Dockerfile-VCS ../K8S/Repositories/External &&
...
kubectl delete deployments repositories-git-remote-service;

#kubectl delete pvc repositories-persistent-volume ;
...
kubectl apply -f ../K8S/Repositories/Service/git-server.yaml &&

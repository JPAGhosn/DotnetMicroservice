...
kubectl delete deployments auth-service auth-database;
kubectl delete services auth-node-port auth-database-node-port;
kubectl delete secrets postgresauth;

kubectl create secret generic postgresauth --from-literal=POSTGRES_PASSWORD=admin
